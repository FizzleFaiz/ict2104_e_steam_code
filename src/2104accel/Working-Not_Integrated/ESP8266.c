...
/*Retrieve response from ESP8266*/
bool ESP8266_WaitForAnswer(uint32_t Tries)
{
    ...
    while(Tries)
    {
        ...
        Tries--;
        __delay_cycles(2400);   //delay to populate ESP8266_Buffer
    }

    return false;
}

/*Checks if data receive is complete*/
...
bool ESP8266_SetUpAP(void)
{
    char ssid[] = "Whatchu_Say";                                                                        //Device Name
    char ssidpassword[] = "jamesderulo";                                                                //Device Password
    char rfchl[] = "11";                                                                                //frequency channel
    char security = '0';                                                                                //security level
    MSPrintf(EUSCI_A2_BASE, "%s=\"%s\",\"%s\",%s,%c\r\n", AT_CWSAP, ssid,ssidpassword,rfchl,security);  //write cmd to ESP8266
    __delay_cycles(48000000);                                                                           //delay to retrieve buffer
...
/*Establish connection to ESP8266*/

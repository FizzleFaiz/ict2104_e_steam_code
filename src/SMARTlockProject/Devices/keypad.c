...
void keypadpress(void){
    ...
    if (GPIO_STATUS & C1_PIN)
    {
        keypadcolcheck(GPIO_STATUS, C1_PIN, R1C1, R2C1, R3C1, R4C1);
    }
    else
    {
        GPIO_STATUS = GPIO_getInputPin_Value(C2_PORT, C2_PIN);
    }
    if (GPIO_STATUS & C2_PIN)
    {
        keypadcolcheck(GPIO_STATUS, C2_PIN, R1C2, R2C2, R3C2, R4C2);
    }
    else
    {
        GPIO_STATUS = GPIO_getInputPin_Value(C3_PORT, C3_PIN);
    }
    if (GPIO_STATUS & C3_PIN)
    {
        keypadcolcheck(GPIO_STATUS, C3_PIN, R1C3, R2C3, R3C3, R4C3);
    }
